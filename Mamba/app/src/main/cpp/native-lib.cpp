...
//Java_com_mamba_model_record_randerer_CameraRenderer_convertRgba2Yuv(JNIEnv *env, jclass type_,
//                                                                    jbyteArray rgba_, jint width,
//                                                                    jint height, jbyteArray yuv_,
//                                                                    jint type) {
...
Java_com_mamba_model_record_randerer_CameraRenderer_convertRgba2Yuv(JNIEnv *env, jclass type_,
                                                                    ...
                                                                    jint height, jbyteArray yuv_,
                                                                    jint type) {
    jbyte *rgbaBuffer = env->GetByteArrayElements(rgba_, 0);
    unsigned char *cRgba = (unsigned char *) rgbaBuffer;
    jbyte *yuvBuffer = env->GetByteArrayElements(yuv_, 0);
    unsigned char *cYuv = (unsigned char *) yuvBuffer;
    rgbaToYuv(width, height, cRgba, cYuv, type);
    env->ReleaseByteArrayElements(rgba_, rgbaBuffer, 0);
    env->ReleaseByteArrayElements(yuv_, yuvBuffer, 0);
}
