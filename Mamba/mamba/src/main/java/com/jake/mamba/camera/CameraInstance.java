...
public final class CameraInstance {
    ...
    public void checkAndInit(Context context) {
        if(mAppContext==null){
            init(context);
        }
    }
...
//            startPreview();
        ...
        public void onError(CameraDevice cameraDevice, int error) {
            ...
            cameraDevice.close();
            mDevice = null;
            callbackCameraError(error, "onError");
        }
        ...
        private void createCaptureSession() {
            ...
            log("createCaptureSession 1");
            try {
                mDevice.createCaptureSession(surfaceList, mCaptureSessionStateCallback, mListenerHandler);
                log("CameraAccessException 2");
            } catch (CameraAccessException e) {
                log("CameraAccessException e"+e.getMessage());
                e.printStackTrace();
            }
    ...
    private void callbackCameraError(int code, String msg) {
        if (mCameraCallbackList != null) {
            for (CameraCallback callback : mCameraCallbackList) {
                if (callback != null) {
    ...
    CameraCaptureSession.StateCallback mCaptureSessionStateCallback = new CameraCaptureSession.StateCallback() {
        ...
        public void onConfigured(CameraCaptureSession session) {
            ...
            try {
                mCameraCaptureSession.setRepeatingRequest(mPreviewBuilder.build(), null, mListenerHandler);
            } catch (CameraAccessException e) {
                e.printStackTrace();
            }
            if (mCameraCallbackList != null) {
                for (CameraCallback callback : mCameraCallbackList) {
                    if (callback != null) {
                        callback.onOpened(mPreviewSize.getWidth(), mPreviewSize.getHeight());
                    }
                }
            }
        }
...
//        rect.bottom = clamp((int) (y + tapAreaRatio / 2 * cropRegion.height()), 0, cropRegion.height());
    ...
    private void handleOpenCamera() {
        if (!hasAvailableCamera) {
            callbackCameraError(CameraCallback.ERROR_OPEN_FAIL, "camera init fail");
            return;
        }
        ...
        try {
            if (!mCameraOpenCloseLock.tryAcquire(2500, TimeUnit.MILLISECONDS)) {
                callbackCameraError(CameraCallback.ERROR_CAMERA_LOCKED, "Time out waiting to lock camera opening.");
            }
            mCameraManager.openCamera(mCurrentCameraId, mStateCallback, mListenerHandler);
        } catch (CameraAccessException e) {
            e.printStackTrace();
            callbackCameraError(CameraCallback.ERROR_OPEN_FAIL, "camera open fail");
        } catch (InterruptedException e) {
            e.printStackTrace();
            callbackCameraError(CameraCallback.ERROR_CAMERA_LOCKED, "camera open fail");
    ...
    public void registerPictureCallback(ImageReader.OnImageAvailableListener pictureCallback) {
    ...
    public void unregisterPictureCallback(ImageReader.OnImageAvailableListener pictureCallback) {
        if (mPictureCallbackList != null) {
            mPictureCallbackList.remove(pictureCallback);
        }
    }

    public void unregisterCameraCallback(CameraCallback callback) {
        if (mCameraCallbackList != null) {
            mCameraCallbackList.remove(callback);
        }
    }

    public void registerCameraCallback(CameraCallback callback) {
        if (callback != null) {
            if (mCameraCallbackList == null) {
    ...
    public void registerPreviewCallback(ImageReader.OnImageAvailableListener previewCallback) {
    ...
    public void unregisterPreviewCallback(ImageReader.OnImageAvailableListener previewCallback) {
        if (mPreviewCallbackList != null) {
            mPreviewCallbackList.remove(previewCallback);
        }
    }
    ...
    private Size getLargeSize(Size[] list, int width, int height) {
    ...
    public interface CameraCallback {
        int ERROR_NO_PERMISSION = -0x01;
        int ERROR_OPEN_FAIL = -0x02;
        int ERROR_CAMERA_LOCKED = -0x02;
